...
def build_model(model_name: Text, inputs: tf.Tensor, **kwargs):
  ...
  """
  print("inference.build_model ran")
  ...
  if kwargs.get('use_keras_model', None):
    ...
    def model_arch(feats, model_name=None, **kwargs):
      ...
      print("model_arch ran in inference")
      ...
      print("\n>>>cls_out_list :::" + str(cls_out_list))
      print("\n>>>box_out_list :::" + str(box_out_list))
      ...
      print("\n>>>cls_outputs :::" + str(cls_outputs))
      print("\n>>>box_outputs :::" + str(box_outputs))
  ...
  print(">>>cls_outputs :::" + str(cls_outputs))
  print(">>>box_outputs :::" + str(box_outputs))
  return cls_outputs, box_outputs
...
def visualize_image(img_id, image_number, image,
                    boxes,
                    classes,
                    scores,
                    label_map=None,
  ...
  """
  print("visualize_image runs")
  label_map = label_util.get_label_map(label_map or 'coco')
  category_index = {k: {'id': k, 'name': label_map[k]} for k in label_map}
  img = np.array(image)
  print("\n***MIN_SCORE_THRESH:::::"+str(min_score_thresh))

  # f = furkan.Furkan(boxes, classes, scores, label_map)
  f = pic_processor.Furkan(img_id, image_number, boxes, classes, scores, label_map, img)
  f.for_Furkan()
  ...
  # dict = f.for_Furkan()
  # print(dict)
  # print("VISUALIZE_IMAGE RUNNING")
  # print("1 IMAGE:::" +str(image))
  # print("2 BOXES:::" +str(boxes))
  # print("3 CLASSES:::" +str(classes))
  # print("4 SCORES:::" +str(scores))
  # print("5 LABEL_MAP:::" +str(label_map))
  # print("6 MIN_SCORE_THRESH:::" +str(min_score_thresh))
  # print("7 MAX_BOXES_TO_DRAW:::" +str(max_boxes_to_draw))
  # print("8 LINE_THICKNESS:::" +str(line_thickness))
  # print("9 **KWARGS???:::" +str(**kwargs))
  return img
...
#                                label_map=None,
#                                **kwargs):
def visualize_image_prediction(img_id, image_number,image,
                               prediction,
                               label_map=None,
                               **kwargs):
  ...
  Args:
    ...
    prediction: a list of vector, with each vector has the format of [image_id, ymin, xmin, ymax, xmax, score, class].
    label_map: a map from label id to name.
    **kwargs: extra parameters for vistualization, such as min_score_thresh, max_boxes_to_draw, and line_thickness.
  ...
  scores = prediction[:, 5]
  print("visualize_image_prediction ran")

  return visualize_image(img_id,image_number,image, boxes, classes, scores, label_map, **kwargs)
...
class ServingDriver(object):
  ...
  Example 2. Serving batch image contents:
    ...
    driver = inference.ServingDriver('efficientdet-d0', '/tmp/efficientdet-d0', batch_size=len(imgs))
    driver.build()
  ...
  def __init__(self,
    ...
    """
    print("ServingDriver __init__ ran")
...
class InferenceDriver(object):
  ...
  def __init__(self,
    ...
    Args:
      ...
      model_params: model parameters for overriding the config.
    """
    print("InferenceDriver __init__ ran")
    self.model_name = model_name
    self.ckpt_path = ckpt_path
    self.params = hparams_config.get_detection_config(model_name).as_dict()
    if model_params:
...
# print("\n\n^^^^^^^^^^^^^^^^^^^^ IDK start ^^^^^^^^^^^^^^^^^^^")
# pic_folder = Path('/Users/myatwin/REU/Project/ObjRec/automl/efficientdet/data_pics/DataSet1').rglob('*.jpg')
#
...
#     imgs.append(np.array(Image.open(f)))
#
# driver = ServingDriver('efficientdet-d5', '/tmp/efficientdet-d5', batch_size=len(imgs))
# driver.build()
# predictions = driver.serve_images(imgs)
# for i in range(len(imgs)):
#     print("for loop: " + str(i))
#     driver.visualize(imgs[i], predictions[i])
#     driver.export('/Users/myatwin/REU/Project/ObjRec/automl/efficientdet/data_pics/processedPics')
#
# print("^^^^^^^^^^^^^^^^^^^^ IDK end ^^^^^^^^^^^^^^^^^^^\n\n")