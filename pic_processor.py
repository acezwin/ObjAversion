...
import inference
# from inference import ServingDrive    r
...
class Furkan:
    ...
    def for_Furkan(self):
        ...
        id_count = 0
        dict = {}
        field_name = ["image_name", "image_id", "objectID", "class_number", "class_name", "y_min", "x_min", "y_max",
                      "x_max", "accuracy", "distance_pred"]
        ...
        while i < len(self.scores):
            ...
            if self.scores[i] >= 0.6:
                ...
                class_number = self.classes[
                    i]  # label for the type of object detected (ex '1' is the label  for 'human')
                ...
                distance_pred = self.model.predict([[y_min, x_min, y_max, x_max, certainty,
                                                     class_number]])  # predictors = ['y_min', 'x_min', 'y_max', 'x_max','prediction', 'class_number']
    ...
    def detect_objcts(img):
        ...
        with tf.Session() as sess:
            ...
            for ind, ff in enumerate(files):
                ...
                up_img = inference.visualize_image_prediction(ind, img_name, raw_images[ind], detections[0],
                                                              label_map=label_map, min_score_thresh=0.6)
                image_id += 1
                # pdb.set_trace()
                # saving output
...
def main_2():
    walker = smartWalker()
    img = "./data_pics/testPics/1623705423.5.png"
    boxes, classes, scores, distances = walker.process_img(img, visualize=True)
    # pdb.set_trace()
    walker.give_direction(boxes, distances)
    # pdb.set_trace()
...
class smartWalker:
    ...
    def give_direction(self, boxes, distances):
        for ind, [y_min, x_min, y_max, x_max] in enumerate(boxes):
            if (float(distances[ind]) < 72):
                print(distances[ind])
                if (float(y_max) > 1.5 * float(x_max)):
                    print("obj on left")
                if (float(y_max) > (-1.5)* (float(x_min)) + 960):
                    print("obj on right")
                elif ((float(y_max) > 1.5 * float(x_min)) and (float(y_max) > (-1.5)* (float(x_max)) + 960)):
                    print("obj infront")
